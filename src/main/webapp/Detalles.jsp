...
<head>
    ...
    <script src="scripts/VozDet.js" type="text/javascript"></script>
</head>
<body>
    ...
    <table>
        ...
        <tbody>
            <% 
                ...
                List<DetalleVenta> detalle = detalleventaDAO.obtenerDetalleVentaPorId(id);
                for (DetalleVenta detallex : detalle) { 
                
                VentaDAO ventaDAO = new VentaDAO();
                List<Venta> venta = ventaDAO.obtenerVentaPorIdVenta(detallex.getIdVenta());
                for (Venta ventas : venta) { 
            %>
            <tr>
                <td><%= detallex.getIdVenta() %></td>
                <td><%= ventas.getFecha()%></td>
                <td><%= detallex.getVentas()%></td>
                <td><%= detallex.getCantidad()%></td>
                <td><%= detallex.getPrecioUnitario()%></td>
                <td><%= detallex.getSubtotal() %></td>
                <td><%= ventas.getMetodoPago()%></td>
                <td><%= detallex.getEstado()%></td>
                
                <td>
                    <form action="generarPDF" method="get" style="display:inline;">
                        <input type="hidden" name="id" value="<%= detallex.getIdVenta() %>">
