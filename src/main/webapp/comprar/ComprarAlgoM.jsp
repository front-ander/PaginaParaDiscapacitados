...
<%
    ...
    MascotaDAO mascotaoDAO = new MascotaDAO();
    Mascota mascot = mascotaoDAO.obtenerPorId(id);
%>
...
<html>
    <head>
        ...
        <script src="../scripts/VozCP_11.js" type="text/javascript"></script>
    </head>
    ...
    <body>
        ...
        <main>
            ...
            <div class="purchase-container">
                <div style="display: flex; flex-direction: row;  width: 1000px; height: auto">
                    <div> <!-- 2 -->
                        <div class="image-section">
                            <img src="data:image/jpeg;base64,<%= mascot.getImagenBase64()%>" alt="Imagen de la mascota" style="width: auto; height: 200px"/>
                        </div >  <!-- imagen -->
                        <div >
                            <input type="hidden" name="id" value="<%= mascot.getID() %>">
                            <div class="info-item" style=" margin-right: 5px"><strong><h3>Categoría:</strong> <%= mascot.getCategoria()%></h3></div>
                            <div class="info-item" style=" margin-right: 5px"><strong><h3>Tiempo:</strong> <%= mascot.getTiempo()%></h3></div>
                            <div class="info-item" style=" margin-right: 5px"><strong><h3>Precio:</strong> S/<%= mascot.getPrecio()%></h3></div>
                        </div> <!-- datos -->
                        
                        <form action="../ComprarAlgoM" method="POST">
                            <input type="hidden" name="id" value="<%= mascot.getID() %>">
                        <button type="submit" id="confirmButton">Comprar</button>
                        </form>
                        <form action="../CarritoM" method="POST">
                            <input type="hidden" name="id" value="<%= mascot.getID() %>">
                        <button type="submit"  id="confirmButtonC">Carrito</button>
                        </form>
                        <button type="submit" onclick="window.location.href='../mascotas.jsp';" id="cancelButton">Cancelar</button>
                        
                    </div> <!-- 2 -->
                    <div> <!-- 3 -->
                        <div style="display: flex; flex-direction: row; justify-content: space-between; align-items: center;"> <!-- 4 -->
                            <div class="info-item">
                                    <h2><strong>Nombre:</strong> <%= mascot.getNombre() %></h2>
                                </div> <!-- nombre -->
                                <div class="info-item" style="background-color: #777; border-radius: 10px">
                                    <h4 style="margin: 3px; padding: 3px;"><strong></strong> <%= mascot.getCalificacion() %> ✩</h4>
                                </div> <!-- categoria -->
                        </div> <!-- 4 -->
                        <div class="info-item" >
                            <h2> <%= mascot.getDescripcionC()%></h2>
                        </div>
                    </div> <!-- 3 -->
                </div> <!-- 1 -->
            </div> <!-- purchase -->
 ...
 <footer>
    ...
    const producto = {
        ...
        descripcion: "<%= mascot.getDescripcionC() %>",
        calificacion: "<%= mascot.getCalificacion()%>",
        precio: "<%= mascot.getPrecio() %>"
    };
</script>
    </body>
</html>
