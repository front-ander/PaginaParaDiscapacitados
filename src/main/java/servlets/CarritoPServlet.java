...
@WebServlet("/CarritoP")
public class CarritoPServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Producto prod = productoDAO.obtenerPorId(idservicio);
        if (prod == null) {
            // Si no se encuentra la mascota, redirigir a una página de error o mostrar mensaje
            response.sendRedirect("../ComprarAlgoP.jsp");
            return;
        }
...
// Aquí puedes establecer otros atributos de la venta, como el método de pago o el total
    ...
    if (exito) {
        ...
        response.sendRedirect("/PasandoOW/comprar/ComprarAlgoP.jsp?mensaje=Error al añadir detalles");
    }
} else {
    ...
    if (exito) {
        ...
        response.sendRedirect("/PasandoOW/comprar/ComprarAlgoP.jsp?mensaje=Error al crear el carrito");
    }
}
}
    }
