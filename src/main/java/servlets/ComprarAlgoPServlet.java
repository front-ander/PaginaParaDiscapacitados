...
@WebServlet("/ComprarAlgoP")
public class ComprarAlgoPServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (prod == null) {
            ...
            response.sendRedirect("../ComprarAlgoP.jsp?mensaje=Mascota no encontrada");
            return;
        }
        ...
        venta.setTotal(prod.getPrecio()*cantidad); // El total es igual al precio de la mascota en este caso
    ...
    if (ultimoIds != null && ultimoIds.startsWith("DVN")) {
        ...
        detalle.setVentas(idproducto);
        detalle.setCantidad(cantidad); // Cantidad fija en 1 ya que es una mascota
        detalle.setPrecioUnitario(prod.getPrecio());
        detalle.setSubtotal(prod.getPrecio()*cantidad); // Subtotal es igual al precio en este caso
        ...
        if (exito) {
            ...
            response.sendRedirect("../comprar/ComprarAlgoP.jsp?mensaje=Error al realizar la compra");
        }
    }
